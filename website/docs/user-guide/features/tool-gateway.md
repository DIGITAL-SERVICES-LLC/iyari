...
```yaml
web:
  use_gateway: false   # IYARI now uses FIRECRAWL_API_KEY from .env
```
...
No — keep them in `.env`. When `use_gateway: true`, IYARI skips direct keys and uses the gateway. Flip the flag back to `false` and your keys become the source again. The gateway isn't a lock-in.
