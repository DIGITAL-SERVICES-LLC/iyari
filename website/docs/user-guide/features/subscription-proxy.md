...
chat completions — use your IYARI-managed provider subscription as their
...
| Use case | "Use IYARI as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. IYARI stores
the refresh token in `~/.hermes/auth.json` — the same place all IYARI
